...
if ($#ARGV != 7) {
	print STDERR "usage: $0 <bedpe> <baitmap file> <rmap file> <output> <read1 chr col> <read1 coord col> <read 2 chr col> <read 2 coord col>\n";
	exit 1;
}
...
while (my $line = <IN>) {
	...
	$frags{$f[3]} = {chrom=>$chr, start=>$f[1], end=>$f[2]};
	$coord2index{$chr}->{$f[3]} = {start=>$f[1], end=>$f[2]};
}
close IN or die $!;
...
while (my $line = <IN>) {
	...
	my $baitfrag;
	my $oefrag;
	if (exists $baits{$leftfrag}) {
		++$bcount;
		$baitfrag = $leftfrag;
		$oefrag = $rightfrag;
	}
	if (exists $baits{$rightfrag}) {
		++$bcount;
		$baitfrag = $rightfrag;
		$oefrag = $leftfrag;
	}
	next unless ($bcount == 1);
	++$kept;
	if (exists $ints{$baitfrag}->{$oefrag}) {
		++$ints{$baitfrag}->{$oefrag};
	}
	else {
		$ints{$baitfrag}->{$oefrag}=1;
		push @{$intrep{$baitfrag}},$oefrag;
	}
}
close IN or die $!;
...
my $cis;
print "Writing ChiCMaxima input file...\n";
open OUT,">",$ofn or die $!;
print OUT "ID_Bait\tchr_Bait\tstart_Bait\tend_Bait\tBait_name\tID_OE\tchr_OE\tstart_OE\tend_OE\tOE_name\tN\n";
my @baitsort = sort {$a <=> $b} keys %ints;
foreach my $bait(@baitsort) {
	my @oesort = sort {$a <=> $b} @{$intrep{$bait}};
	$cis += scalar @oesort;
	foreach my $oe(@oesort) {
		print OUT $bait,"\t",$frags{$bait}->{chrom},"\t",$frags{$bait}->{start},"\t",$frags{$bait}->{end};
		print OUT "\t",$baits{$bait},"\t",$oe,"\t",$frags{$oe}->{chrom},"\t",$frags{$oe}->{start},"\t",$frags{$oe}->{end};
